...
private:
    ...
    TaskThreadPool *_business_thread_pool;                                      // 业务线程池,为空则在EventLoop中处理业务
    void _Release();                                                            // 释放连接
    void _RunMessageCallback(const std::function<void()> &done = nullptr);      // 调用业务处理回调
...
public:
    ...
    Connection(EventLoop *event_loop, uint64_t id, Socket &&sock,
               TaskThreadPool *business_thread_pool = nullptr); // 构造函数,参数为连接ID和套接字对象
    ~Connection();
